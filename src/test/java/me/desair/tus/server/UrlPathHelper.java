 ...
 * {@link org.springframework.web.servlet.support.RequestContext} for path
 * matching and/or URI determination.
 *
...
public class UrlPathHelper {
     ...
     * simply because we need the very first URI in the request forwarding
     * chain.
     */
     ...
     * <p>
     * If set to {@literal false} the path within the current servlet mapping is
     * used instead if applicable (i.e. in the case of a prefix based Servlet
    ...
    /**
     * Whether configured to remove ";" (semicolon) content from the request
     * URI.
     */
    ...
    /**
     * Set the default character encoding to use for URL decoding. Default is
    ...
    /**
     * Switch to read-only mode where further configuration changes are not
     * allowed.
     */
     ...
     * which is the case when we can establish that the Servlet is not mapped by
     * servletPath prefix.
     */
     ...
     * the part of the request's URL beyond the part that called the servlet, or
     * "" if the whole URL has been used to identify the servlet.
     *
     * @param request current HTTP request
     ...
     * Detects include request URL if called within a RequestDispatcher include.
     * <p>
     * E.g.: servlet mapping = "/*"; request URI = "/test/a" &rarr; "/test/a".
     * <p>
     * E.g.: servlet mapping = "/"; request URI = "/test/a" &rarr; "/test/a".
     * <p>
     * E.g.: servlet mapping = "/test/*"; request URI = "/test/a" &rarr; "/a".
     * <p>
     * E.g.: servlet mapping = "/test"; request URI = "/test" &rarr; "".
     * <p>
     * E.g.: servlet mapping = "/*.test"; request URI = "/a.test" &rarr; "".
     *
     * @param request current HTTP request
    ...
    /**
     * Return the servlet path for the given request, regarding an include
     * request URL if called within a RequestDispatcher include.
     * <p>
     * As the value returned by {@code request.getServletPath()} is already
     * decoded by the servlet container, this method will not attempt to decode
     * it.
     *
     * @param request current HTTP request
    ...
    /**
     * Return the request URI for the given request. If this is a forwarded
     * request, correctly resolves to the request URI of the original request.
     */
    ...
    /**
     * Return the servlet path for the given request, detecting an include
     * request URL if called within a RequestDispatcher include.
     *
     * @param request current HTTP request
     * @return the servlet path
    ...
    /**
     * Return the query string part of the given request's URL. If this is a
     * forwarded request, correctly resolves to the query string of the original
     * request.
     *
     * @param request current HTTP request
     ...
     * {@linkplain #setRemoveSemicolonContent removeSemicolonContent} property
     * is set to "true". Note that "jsessionid" is always removed.
     *
    ...
    /**
     * Decode the given URI path variables via {@link #decodeRequestString}
     * unless {@link #setUrlDecode} is set to {@code true} in which case it is
